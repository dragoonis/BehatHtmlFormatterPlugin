...
class TwigRenderer
{
    ...
    public function renderAfterExercise($obj) {
        ...
        $print = $twig->render('index.html.twig',
            array(
                ...
                'percentScenariosPassed' => $this->getNumPercentScenariosPassed($obj),
                'percentScenariosFailed' => $this->getNumPercentScenariosFailed($obj)
            )
        );
     ...
     * @return float|int
     */
    private function getNumPercentScenariosPassed($obj)
    {
        $numFailedScenarios = count($obj->getFailedScenarios());
        $numPassedScenarios = count($obj->getPassedScenarios());
        $totalScenarios = $numFailedScenarios + $numPassedScenarios;

        if($totalScenarios <= 0) {
            return 0;
        }

        return round($numPassedScenarios / ($totalScenarios / 100), 2);
     ...
     * @return float|int
     */
    private function getNumPercentScenariosFailed($obj)
    {
        $numFailedScenarios = count($obj->getFailedScenarios());
        $numPassedScenarios = count($obj->getPassedScenarios());
        $totalScenarios = $numFailedScenarios + $numPassedScenarios;
        ...
        return round($numFailedScenarios / ($totalScenarios / 100), 2);
